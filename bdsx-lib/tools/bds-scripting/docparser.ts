...
const OUT_SCRIPTING = 'generated.scripting.d.ts';
const OUT_ADDONS = 'generated.addons.d.ts';
...
async function parseScriptingDoc():Promise<void> {
    ...
    const writer = new FileWriter(path.join(targetDir, OUT_SCRIPTING));
    const lines = StringLineWriter.generateWarningComment('bdsx-dev/bds-scripting/parser.ts');
    await writer.write('/**\n * '+lines.join('\n * ')+'\n */\n\n');
...
async function parseAddonsDoc():Promise<void> {
    ...
    const writer = new FileWriter(path.join(targetDir, OUT_ADDONS));
    const lines = StringLineWriter.generateWarningComment('bdsx-dev/bds-scripting/parser.ts');
    await writer.write('/**\n * '+lines.join('\n * ')+'\n */\n\n');
    await writer.write(`declare global {\n\n`);
