...
class UnionBuilder {
    private readonly types = new Set<PdbId<PdbId.Data>>();

    add(other:PdbId<PdbId.Data>):boolean {
        if (other.is(PdbId.TypeUnion)) {
            for (const item of other.data.unionedTypes) {
                if (!this.add(item)) {
                    return false;
                }
            }
    ...
    make(unionOperator:'|'|'&'):PdbId<PdbId.TypeUnion> {
        const array = [...this.types].sort((a,b)=>a.id-b.id);
...
export class PdbId<DATA extends PdbId.Data> {
    ...
    hasArrayTemplateParam():boolean {
        if (this.templateParameters === null) return false;
        for (const param of this.templateParameters) {
    ...
    unionWith(other:PdbId<PdbId.Data>):PdbId<PdbId.Data>{
        ...
        const unionedTypes = new UnionBuilder;
        if (!unionedTypes.add(a)) {
            return PdbId.any_t;
        }
        if (!unionedTypes.add(b)) {
            return PdbId.any_t;
        }
        return unionedTypes.make('|');
    }

    static makeUnionedType(items:PdbId<PdbId.Data>[], unionOperator:'&'|'|'='|'):PdbId<PdbId.TypeUnion> {
        const unionedTypes = new UnionBuilder;
        for (const item of items) {
            if (!unionedTypes.add(item.removeConst())) {
                return PdbId.any_t;
            }
        }
        return unionedTypes.make(unionOperator);
    }
    ...
    getPointerTypeOfIt():PdbId<PdbId.Data> {
        try {
            return this.data.getPointerTypeOfIt();
        } catch (err) {
            console.error(`> getPointerTypeOfIt`);
            throw err;
        }
    }
...
export namespace PdbId {
    ...
    export class TemplateFunctionBase extends FunctionBase implements HasOverloads {
        ...
        *allOverloads():IterableIterator<PdbId<PdbId.Function>> {
        ...
        instanceOf<T extends Data>(type:AbstractClass<T>):this is T {
            return this instanceof type || isBaseOf(TemplateBase, type);
        }
        convertTo<T extends Data>(type:new(id:PdbId<any>)=>T):T {
            if (type === TemplateBase as any) {
                const data = new TemplateFunctionBase(this.id);
        ...
        getTypeOfIt():PdbId<Data> {
            if (this.id.isValue) {
                if (this.deco === DecoSymbol["&"]) {
                    return this.base.getPointerTypeOfIt();
                }
            }
    ...
    export const addressMap = new Map<number, PdbId<PdbId.Data>[]>();
...
function parse(list?:number[]):void {
    ...
    for (const {address, name} of cache) {
        ...
        try {
            ...
            id.address = address;
            const arr = PdbId.addressMap.get(address);
            if (arr == null) PdbId.addressMap.set(address, [id]);
            else arr.push(id);
