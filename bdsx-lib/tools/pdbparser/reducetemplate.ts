...
class ComparingBase {
    ...
    static equals(target:Identifier|null|undefined, base:Identifier|null|undefined):Identifier|null {
        ...
        if (target.templateBase !== null) {
            ...
            const btemplates = base.templateParameters!;
            if (atemplates.length !== btemplates.length) return null;
            for (let i=0;i<atemplates.length;i++) {
                const atemplate = atemplates[i];
                const btemplate = btemplates[i];
                const res = ComparingBase.equals(atemplate, btemplate);
                if (res === null) return null;
                if (btemplate !== res) {
                    if (bparams === null) bparams = btemplates.slice();
                    bparams[i] = res;
                }
            }
