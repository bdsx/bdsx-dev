...
export class NetworkIdentifier extends NativeClass implements Hashable {
    ...
    static fromNewNi(ptr:minecraft.NetworkIdentifier&{[legacyLink]?:NetworkIdentifier}):NetworkIdentifier {
        const legacy = ptr[legacyLink];
        if (legacy != null) return legacy;
        return ptr[legacyLink] = ptr.as(NetworkIdentifier);
    }
...
Object.defineProperty(exports, 'networkHandler', {
    get(){
        const networkHandler = mcglobal.networkHandler.as(NetworkHandler);
...
bdsx.events.playerDisconnect.on(ev=>{
    try {
        events.networkDisconnected.fire(NetworkIdentifier.fromNewNi(ev.player.getRawNetworkIdentifier()));
        _tickCallback();
    } catch (err) {
