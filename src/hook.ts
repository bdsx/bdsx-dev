...
export function hook<THIS, NAME extends keyof THIS, TYPES extends Type<any>[]>(nf:{name:string, prototype:THIS}|null, name:NAME, ...types:TYPES):hook.Tool<THIS, UnwrapFunc<THIS[NAME], TYPES>>;

/**
 * @returns returns 'hook.fail' if it failed.
 */
export function hook(nf:AnyFunction|null, name?:keyof any|Type<any>|null, ...types:Type<any>[]):hook.Tool<any, AnyFunction> {
    if (nf === null) {
    ...
    return new hook.Tool<any, AnyFunction>(nf, String(name), thisType);
}
