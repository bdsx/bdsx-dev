...
import { emptyFunc } from "../bdsx/bdsx/common";
import { remapAndPrintError } from "../bdsx/bdsx/source-map-support";
import { arrayEquals } from "../bdsx/bdsx/util";
...
const outDir = path.join(__dirname, '../bdsx/bdsx');
...
namespace imports {
    ...
    export const enums = new TsFile('./enums');
...
class ParsedFunction {
    ...
    constructor(file:MinecraftTsFile, name:tsw.Property, field:PdbMember<PdbId.Function>, overload:Identifier) {
        ...
        if (overload.templateBase !== null) {
            ...
            const types = file.toTswParameters(typeOfTemplates);
...
class TsCodeDeclaration extends TsCode {
            ...
            for (const overload of overloads) {
                try {
                    ...
                    if (tinfo.parameters.length !== 0) {
                        const templateDecl = tinfo.makeWrappedTemplateDecl(this.nameMaker);
                        const funcReturn = new tsw.FunctionType(func.declare.returnType || tsw.BasicType.any, func.declare.params.params);
                        const args = templateDecl.params.map(([name, type])=>new tsw.VariableDefineItem(new tsw.Name(name), type || tsw.BasicType.any));
    ...
    private _writeClassMember(member:PdbMember<PdbId.Data>):void {
        try {
            ...
            const clsname = this.getNameOnly(member.base);
            const clsnameValue = clsname.toName().value;
            const cls = new tsw.Class(clsnameValue);
            const tinfo = TemplateInfo.from(member.base);
            if (tinfo.paramTypes.length !== 0) {
                ...
                this.base.currentClassId = member.base;
                opened = true;
            ...
            for (const _ of this.enterNamespace(member.base)) {
                if (!opened) {
                    for (const field of sortedMember) {
                        try {
...
class MinecraftTsFile extends TsFile implements wrapperUtil.TsBase {
    ...
    public readonly CxxVectorToArray = new class extends wrapperUtil.ImportItem {
        protected _allocate(component:tsw.ItemPair):wrapperUtil.WrappedItem{
            const This = this.import();
            return new wrapperUtil.WrappedItem(
                This.value.call(tswNames.make, [component.value]),
                new tsw.ArrayType(component.type),
                This,
                component
            );
        }
    }(this, imports.cxxvector, 'CxxVectorToArray');
                ...
                if (item.hasNonGlobalParent()) {
                    ...
                    if (insideOfNamespace && !opts.absoluteValue) {
                        ...
                        if (!insideOfNamespace) {
                            ...
                            out = out.changeType(out.type.member(prop));
                        } else { // absoluteValue=true
                            out = this.toTsw(item.parent, {noTemplate: true, absoluteValue:true});
                            out = out.changeType(out.type.member(prop));
                        }
                        if (out.value != null) {
                            ...
                            out = out.changeValue(out.value.member(prop));
                        }
                    }
                } else {
                    out = prop.toName();
    ...
    plaining(raw:Identifier, item:tsw.ItemPair, opts:MakeFuncOptions|null):tsw.ItemPair {
        function typePlaining(item:tsw.ItemPair, opts:MakeFuncOptions|null):tsw.ItemPair {
            if (opts !== null) {
                item = item.notNull();
            }
            if (item instanceof wrapperUtil.WrappedItem) {
                return item.changeComponent(typePlaining(item.component, opts));
            }
            return item;
        }
        ...
        try {
            ...
            const plainingInner = (item:tsw.ItemPair):tsw.ItemPair=> {
                ...
                if (this.CxxVectorToArray.is(item)) {
                    ...
                    item = item.changeBoth(
                        this.CxxVectorToArray.importValue().call(tswNames.make, [component.value]),
                        new tsw.ArrayType(component.type)
                    );
                }
                if (item instanceof wrapperUtil.WrappedItem) {
    ...
    save():void {
        ...
        head.unshift(...[
            ...
            ...ScriptWriter.generateWarningComment('bdsx-dev/pdbparser/symbolwriter.ts')
...
(PdbId.parse('std::basic_string<char,std::char_traits<char>,std::allocator<char> >') as Identifier).filted = true;
