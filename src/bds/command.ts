...
CommandContext.prototype.constructWith = minecraft.CommandContext.prototype.constructWith as any;
...
export class CommandRegistry extends HasTypeId {
     ...
     * @alias registerOverloadClass
     */
    registerOverload(name:string, commandClass:{new():Command}, params:CommandParameterData[]):void {
        return this.registerOverloadClass(name, commandClass, params);
    }

    /**
     * CAUTION: this method will destruct all parameters in params
     */
    registerOverloadClass(name:string, commandClass:{new():Command}, params:CommandParameterData[]):void {
    ...
    static getParser<T>(type:Type<T>):VoidPointer {
        const parser = dnf(minecraft.CommandRegistry.prototype.parse).getByTemplates(null, type);
        if (parser == null) throw Error(`${type.name} parser not found`);
        return dnf(parser).getAddress();
    }
}
