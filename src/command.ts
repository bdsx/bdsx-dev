...
import { Command, CommandCheatFlag, CommandContext, CommandOutput, CommandParameterData, CommandPermissionLevel, CommandRegistry, CommandRegistry as CommandRegistryLegacy, CommandUsageFlag, CommandVisibilityFlag, MCRESULT, MinecraftCommands } from './bds/command';
...
export class CustomCommandFactory {
    constructor(
        public readonly registry:CommandRegistry|CommandRegistryLegacy,
    ...
    overload<PARAMS extends Record<string, Type<any>|[Type<any>, boolean]>>(
        ...
        this.registry.registerOverloadClass(this.name, CustomCommandImpl as any, params as any);
        return this;
    }
...
function executeCommand(this:MinecraftCommands, ctxptr:SharedPtr<CommandContext>, b:bool_t):MCRESULT {
    try {
        const ctx = ctxptr.p!;
        ...
        default:
            _tickCallback();
            return executeCommandOriginal.call(this, ctxptr, b);
        }
    } catch (err) {
        events.errorFire(err);
...
const executeCommandOriginal = hook(MinecraftCommands, 'executeCommand').call(executeCommand);
